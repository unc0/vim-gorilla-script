...
setlocal indentkeys+=0],0),0.,=else,=case,=default,=catch,=finally
...
if exists('g:gorilla_indent_keep_current')
...
let s:COMPOUND_EXPRESSION = '\C[:=]\s*\%(if\|unless\|for\|while\|until\|'
...
let s:FUNCTION = '\C\%(let\s\+\%(mutable\)\?\s*\I\%(\i\|-\)*(.*)\|'
\              . 'def\s\+\I\%(\i\|-\)*\s*(.*)\|'
\              . '[:=]\s*\%(#\|#\s*(.*)\)\)[!@^&%*]*$'
...
let s:BREAK_BLOCK_OP = '\C^\%(return\|returnif\|break\|continue\|throw\)\>'
...
let s:KEYWORD_PAIRS = [
\  ['\C^else\>', '\C\<\%(if\|unless\|else\s\+\%(if\|unless\)\)\>',
\   '\C\<else\>'],
\  ['\C^default\>', '\C\<\%(switch\|case\)\>', '\C^default\>'],
\  ['\C^catch\>', '\C\<\%(try\|else\)\>', '\C\<catch\>'],
\  ['\C^finally\>', '\C\<\%(try\|else\)\>', '\C\<finally\>']
\]
...
function! GetGorillaIndent(curlnum)
  ...
  " Check if the current line is a `case` and not the first in a switch block.
  if curline =~ '\C^case\>' && !s:SearchCode(prevnlnum, '\C\<switch\>')
    " Look back for a `case`.
    while prevnlnum
      if getline(prevnlnum) =~ '\C^\s*case\>'
        " Indent to match the found `case`, but don't force indenting (for
        " case indenting nested switch blocks.)
    ...
    " No matching `case` found (bad syntax), so bail.
    exec 'return' s:GetDefaultPolicy(a:curlnum)
  endif
  ...
  if prevline =~ s:BEGIN_BLOCK
    " Indent if the current line isn't a single-line statement.
  ...
  " Outdent if a keyword breaks out of a block
